...
class SEOAnalyticsDashboard:
    ...
    def add_metric_data(
        ...
        """Add a metric reading and compute change/trend metadata."""

        existing = self.metrics.get(metric_name)
        baseline = previous_value if previous_value is not None else (
            existing.current_value if existing else value
        )
