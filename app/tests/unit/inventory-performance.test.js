...
import { OptimizedVelocityAnalyzer, OptimizedReorderCalculator, PerformanceMonitor, DEFAULT_PERFORMANCE_CONFIG, } from "../../../dashboard/app/lib/inventory/performance";
// Helper function to create mock SKU data
const createMockSku = (id, velocity) => ({
    id,
    title: `Product ${id}`,
    sku: `SKU-${id}`,
...
const createLargeDataset = (size) => {
    return Array.from({ length: size }, (_, i) => createMockSku(`sku-${i}`, Math.random() * 200 + 10));
};
