...
import {
  DemandForecaster,
  VendorPerformanceAnalyzer,
  InventoryInsightGenerator,
} from "../../../dashboard/app/lib/inventory/analytics";
import type { InventorySkuDemand } from "../../../dashboard/app/types/dashboard";

// Helper function to create mock SKU data
const createMockSku = (id: string, velocity: number, status: "healthy" | "low" | "backorder" | "preorder" = "healthy"): InventorySkuDemand => ({
  id,
  title: `Product ${id}`,
  sku: `SKU-${id}`,
...
describe("VendorPerformanceAnalyzer", () => {
  ...
  it("should generate appropriate recommendations", () => {
    ...
    expect(performance[0]?.recommendations.some(rec => 
      rec.includes("delivery") || rec.includes("quality") || rec.includes("lead time")
    )).toBe(true);
  });
});
...
describe("InventoryInsightGenerator", () => {
  ...
  it("should generate velocity insights", () => {
    ...
    const insights = InventoryInsightGenerator.generateInsights(skus);
    const velocityInsights = insights.filter(insight => 
      insight.title.includes("Fast Movers") || insight.title.includes("Slow Movers")
    );
